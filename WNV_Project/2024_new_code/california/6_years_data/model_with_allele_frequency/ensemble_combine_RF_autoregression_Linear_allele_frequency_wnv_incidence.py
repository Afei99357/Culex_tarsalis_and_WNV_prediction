...
df = pd.read_csv("/Users/ericliao/Desktop/WNV_project_files/WNV/california/data/cali_week_impute_monthly_mean_value_with_allele_frequency_RDA_after_drop_high_cor_and_low_target_cor_disease_incidence.csv",
                   index_col=False,
                   header=0)
...
# Get labels
train_rf_labels = train_rf.pop("incident_rate_WNV").values
test_rf_labels = test_rf.pop("incident_rate_WNV").values
...
df_linear = df[['Date', 'incident_rate_WNV']]
...
for i in range(1, 3):
    df_lagged['t-' + str(i)] = df_linear.groupby('FIPS')['incident_rate_WNV'].shift(i)

df_lagged['t'] = df_linear['incident_rate_WNV']
...
df_result.to_csv("/Users/ericliao/Desktop/WNV_project_files/WNV/california/autoregression_combine_RF_and_Linear/"
                 "auto_linear_rf_combine_model_results_count_allel_frequency_all_gene_after_drop_high_correlated_incidence.csv")
...
print("The standard deviation of the target is: ", df["incident_rate_WNV"].std())
