...
rolling_window = 3
train_roll = train.groupby("Year")["Human_Disease_Count"].sum().reset_index()
...
train_roll["rolling_var"] = train_roll["Human_Disease_Count"].rolling(window=rolling_window, min_periods=1).var()

# Plot the results
plt.figure(figsize=(14, 7))
plt.plot(train_roll["Year"], train_roll["Human_Disease_Count"], label='Actual Cases', color="black")
plt.plot(train_roll["Year"], train_roll["rolling_mean"], label='Rolling Mean', linestyle='--', color="red")
plt.plot(train_roll["Year"], train_roll["rolling_std"], label='Rolling Std Dev', linestyle=':', color="blue")
# plt.plot(train_roll["Year"], train_roll["rolling_var"], label='Rolling Variance', linestyle='-.', color="green")
plt.xlabel('Year')
plt.ylabel('WNV Cases')
plt.title(f'WNV Cases with Rolling Statistics for California with {rolling_window}-year Window')
plt.xticks(np.arange(2004, 2024, 1), rotation=45, fontsize=5)
plt.legend(loc='upper right')
plt.savefig(f'/Users/ericliao/Desktop/WNV_project_files/WNV/california/new_data_2004_2023/result/plots/rolling_statistics/california_04_23_rolling_statistics.png', dpi=300)
